...
<template>
    ...
    >
      ...
      <template #footer>
        <van-button v-if="team.userId !== user.id" type="primary" size="small" round @click="doJoinTeam(team.id)">加入队伍</van-button>
        <van-button v-else type="primary" size="small" round @click="editTeam(team.id)">编辑队伍</van-button>
      </template>
    </van-card>
</template>
