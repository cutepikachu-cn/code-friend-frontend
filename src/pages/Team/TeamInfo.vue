...
<template>
  ...
  <van-cell-group title="队伍信息" inset>
    <template #title>
      <div
        ...
        队伍信息
        <div
            @click="router.push(`/team/edit?teamId=${team.id}`)"
             v-if="team.userId === user.id"
    ...
    <van-cell title="人数" :value="team.maxNumber">
  ...
  <van-cell-group title="队伍成员">
    ...
    <van-cell-group title="操作" :style="{padding: '10px'}" v-if="team.userId === user.id">
      <van-button type="danger" block round @click="doDismissTeam">解散队伍</van-button>
    </van-cell-group>
  </van-cell-group>
</template>
